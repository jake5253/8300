...
dhcp_server_stop ()
{
   ...
   prepare_hostname
   prepare_dhcp_conf $SYSCFG_lan_ipaddr $SYSCFG_lan_netmask dns_only
   $SERVER $DEBUG_FLAG -u nobody -P 4096 -C $DHCP_CONF -8 $LOG
...
services_start()
{
   ...
   then
      ...
      if [ "$LOG_LEVEL" = "0" ] ; then
        ...
        $SERVER $DEBUG_FLAG -u nobody --dhcp-authoritative --disable_dhcp_log $DISABLE_DHCP_LOG -P 4096 -C $DHCP_CONF -8 $LOG $ORDER
