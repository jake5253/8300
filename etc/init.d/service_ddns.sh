...
update_noip_server ()
{
   CURRENT_TIME=`get_current_time`
	...
	ddnspass=$(echo ${SYSCFG_ddns_password}|sed s/@/%$ascii_val/)
   local url="https://${username}:${ddnspass}@dynupdate.no-ip.com/nic/update?hostname=${SYSCFG_ddns_hostname}&myip=$EFFECTIVE_WAN_IPADDR"
   MODEL=`syscfg get device::modelNumber`
   HW_REV=`syscfg get device::hw_revision`
   FW_VERSION=`syscfg get fwup_firmware_version`
