   ...
   if [ "pppoe" != "$SYSCFG_wan_proto" ] ; then
	...
	if [ $BBVERSION -gt 11502 ]; then
        ...
        if [ -n "$SYSCFG_vendorclass" -a "$SYSCFG_vendorclass" != "" ] ; then
            option_vendorclass="-V $SYSCFG_vendorclass"
        else
            option_vendorclass=""
        fi
      if [ -z "$UDHCPC_PID" ] ; then
         ulog dhcp_link status "starting dhcp client on wan ($SYSCFG_ifname)"
         udhcpc -R -S -b $option_clientid $option_vendorclass -i $SYSCFG_ifname -h $SYSCFG_hostname -p $UDHCPC_PID_FILE --arping -s $UDHCPC_SCRIPT $UDHCPC_OPTIONS $UDHCPC_EXTRA_PARAMS &
