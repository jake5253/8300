...
case $1 in
    add)
        ...
        if [ ! -z "$disk" ]; then
            ...
            "msdos")
                partedInfo=`echo "$partedOutput" | grep "^ *$disk "`
                partType=`echo "$partedInfo" | awk '{print $5}'`
       ...
       if [ "$PROD_NAME" = "caiman" -o "$PROD_NAME" = "cobra" -o "$PROD_NAME" = "shelby" -o "$PROD_NAME" = "rango" ]; then
    ...
    remove)
        ...
        if [ "$PROD_NAME" = "cobra" -o "$PROD_NAME" = "caiman" -o "$PROD_NAME" = "shelby" -o "$PROD_NAME" = "rango" ]; then
        ...
        if [ "$PROD_NAME" = "blk-mamba" -o "$PROD_NAME" = "cobra" -o "$PROD_NAME" = "caiman" -o "$PROD_NAME" = "shelby" -o "$PROD_NAME" = "rango" ]; then
            case $UMOUNT in
            esata)
