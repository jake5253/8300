...
add_storage_drivers()
{
	...
	if [ "$PROD_NAME" = "impala" -o "$PROD_NAME" = "wraith" -o "$PROD_NAME" = "macan" -o "$PROD_NAME" = "rango" -o "$PROD_NAME" = "civic" -o "$PROD_NAME" = "dallas" -o "$PROD_NAME" = "xtrail" ]; then
		modprobe nls_utf8
		modprobe tuxera-procfs
		modprobe thfsplus
		modprobe tntfs
    else
		modprobe -q usfd
...
rm_storage_drivers()
{
    local dc=`sysevent get no_usb_drives`
    echo "dc=$dc,rm_storage_drivers >>" > /dev/console
    [ $dc -gt 0 ] && return
