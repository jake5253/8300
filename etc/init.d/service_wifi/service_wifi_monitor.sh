...
for PHY_IF in $PHYSICAL_IF_LIST; do
	...
	if [ "`sysevent get ${WL_SYSCFG}_status`" = "up" ] && [ "$DRIVER_STATUS" = "0" ] && [ "`sysevent get wifi_renew_clients-status`" != "starting" ] && [ "`sysevent get wifi_renew_VAP-status`" != "starting" ] && [ "`sysevent get autochannel-status`" != "running" ]; then
