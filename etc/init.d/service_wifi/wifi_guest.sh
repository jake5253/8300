...
configure_guest()
{
	...
	if [ "`cat /etc/product`" = "nodes" ] && [ "$SYSCFG_INDEX" = "wl2" ] ; then
	...
	if [ "`cat /etc/product`" = "nodes" ] ; then
		if [ "$SYSCFG_INDEX" = "wl0" ]; then
			GUEST_PASSPHRASE="wpa_passphrase=`syscfg_get guest_password`"
		else
			GUEST_PASSPHRASE="wpa_passphrase=`syscfg_get "$SYSCFG_INDEX"_guest_password`"
		fi
