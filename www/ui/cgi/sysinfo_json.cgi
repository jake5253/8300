...
if [ "$SECTION" == "" ] || [ "`echo $SECTION | grep 'Rtt'`" ] ; then
	SECTION="Rtt"
	echo "var Rtt = {"
	echo "  \"title\": \"rtt\"," 
	echo "  \"description\": \"rtt test data\"," 
	echo "  \"data\": ["
	COUNTER="0"
	if [ -f "`syscfg get rtt::log`" ]; then
		while IFS='' read -r line || [[ -n "$line" ]]; do		
			TS="`echo $line | cut -d',' -f1`"
			RTT="`echo $line | cut -d',' -f2`"
			CT="`echo $line | cut -d',' -f3`"
			MAC="`echo $line | cut -d',' -f4`"
			TCPI_RTT="`echo $line | cut -d',' -f5`"
			TCPI_ADVMSS="`echo $line | cut -d',' -f6`"
			TCPI_RCV_SSTHRESH="`echo $line | cut -d',' -f7`"
			TCPI_PMTU="`echo $line | cut -d',' -f8`"
			TCPI_RTTVAR="`echo $line | cut -d',' -f9`"
			TCPI_SND_CWND="`echo $line | cut -d',' -f10`"
			TCPI_RND_SSTHREASH="`echo $line | cut -d',' -f11`"
			TCPI_REORDERING="`echo $line | cut -d',' -f12`"
			...
			if [ "$TS" ] ; then
				...
				echo "{\"Date\": \"$TS\","
				echo "$RTT" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$CT" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$MAC" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$TCPI_ADVMSS" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$TCPI_RCV_SSTHRESH" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$TCPI_PMTU" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$TCPI_RTTVAR" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$TCPI_SND_CWND" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$TCPI_RND_SSTHREASH" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo -n "$TCPI_REORDERING" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\"}/g"
				COUNTER="`expr $COUNTER + 1`"
			fi
		done < "`syscfg get rtt::log`"
	else
		SERVER_IP="`syscfg get rtt::serverip`"
		PORTNO="`syscfg get rtt::portno`"
		RTT="`/usr/sbin/rtt_client $SERVER_IP $PORTNO | cut -d' ' -f5`"
...
if [ "$SECTION" == "" ] || [ "`echo $SECTION | grep 'Thrulay'`" ] ; then
	SECTION="Thrulay"
	echo "var Thrulay = {"
	echo "  \"title\": \"thrulay\"," 
	echo "  \"description\": \"thrulay test data\"," 
	echo "  \"data\": ["
	COUNTER="0"
	if [ -f "`syscfg get thrulay::log`" ]; then
		while IFS='' read -r line || [[ -n "$line" ]]; do		
			TS="`echo $line | cut -d',' -f1`"
			IP="`echo $line | cut -d',' -f2`"
			MBPS="`echo $line | cut -d',' -f3`"
			RTT="`echo $line | cut -d',' -f4`"
			JITTER="`echo $line | cut -d',' -f5`"
			MIN="`echo $line | cut -d',' -f6`"
			AVG="`echo $line | cut -d',' -f7`"
			MAX="`echo $line | cut -d',' -f8`"
			...
			if [ "$TS" ] ; then
				...
				echo "{\"Date\": \"$TS\","
				echo "$IP" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$MBPS" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$RTT" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$JITTER" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$MIN" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$AVG" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$MAX" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\",/g"
				echo "$DURATION" | sed 's/ /"/g' | sed 's/:/": "/g' | sed "s/$/\"}/g"
				COUNTER="`expr $COUNTER + 1`"
			fi
		done < "`syscfg get thrulay::log`"
	else
		SERVER_IP="`syscfg get thrulay::serverip`"
		PORTNO="`syscfg get thrulay::portno`"
		echo "{\"Server\": \"$SERVER_IP\","
		echo "\"Port\": \"$PORTNO\""
		echo "}"
	fi
