...
-- $Id: //depot-irv/olympus_partners/dallas_dev/lego_overlay/proprietary/jnap/lualib/wirelessap.lua#7 $
--
...
function _M.getSupportedRadios(sc)
    ...
    for id, value in pairs(_M.RADIO_PROFILES) do
        if supported:find(value.apName, 1, true) then
            ...
            local defaultMixed = sc:get_wifi_default_mixed_mode(radios[id].apName)
            radios[id].defaultMixedMode = defaultMixed and '802.'..defaultMixed
...
function _M.getWirelessRadioMap(sc)
    ...
    for radioID, profile in pairs(_M.getSupportedRadios(sc)) do
        map[radioID] = {
            ...
            defaultMixedMode = profile.defaultMixedMode,
...
function _M.parseWirelessRadioMode(mode)
    ...
    elseif mode == '11b 11g 11n 11ac' then
        return '802.11bgnac'
    elseif mode == '11b 11g 11n 11ax' then
        return '802.11bgnax'
    elseif mode == '11a 11n 11ac 11ax' then
        return '802.11anacax'
...
local function serializeWirelessRadioMode(mode)
    ...
    elseif mode == '802.11anac' then
        return '11a 11n 11ac'
    elseif mode == '802.11anacax' then
        return '11a 11n 11ac 11ax'
    elseif mode == '802.11bgn' then
        return '11b 11g 11n'
    elseif mode == '802.11bgnac' then
        return '11b 11g 11n 11ac'
    elseif mode == '802.11bgnax' then
        return '11b 11g 11n 11ax'
...
function _M.getRadioInfo3(sc)
    ...
    for radioID, radio in pairs(radioMap) do
        local radioOutput = {
            ...
            band = radio.band,
            supportedModes = radio.supportedModes,
            defaultMixedMode = radio.defaultMixedMode,
...
function _M.setWirelessRadioSettings(sc, radioID, settings, apiVersion, copySettingsTo5GhzHigh)
    ...
    if apiVersion == 2 then
        ...
        -- Only 802.11ac, 802.11ax, 802.11anac and 802.11anacax support Wide80/Wide160c/Wide160nc channels.
        if ('802.11ac' ~= settings.mode) and ('802.11ax' ~= settings.mode) and ('802.11anac' ~= settings.mode) and ('802.11anacax' ~= settings.mode) and
