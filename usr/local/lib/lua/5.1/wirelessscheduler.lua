...
function _M.isCurrentTimeBlocked(sc)
    sc:readlock()

    local days = { 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday' }
    -- Calculate the time offset from UTC, based on the local timezone
    local sign, hh, mm = platform.getNumericTimeZone(sc):match('([-+])(%d%d)(%d%d)')
    local offsetSecs = (hh * 3600) + (mm * 60)
    if sign == '-' then
        offsetSecs = -offsetSecs
    end
    -- Get the local date/time and check it against the wireless schedule
    local dt = os.date('*t', os.time() + offsetSecs)
    local timeBlocks = sc:get_wifischeduler_time_blocks(days[dt.wday])
    local index = dt.hour * 2 + 1 + math.floor(dt.min / 30)

    return (timeBlocks:sub(index, index) == '0')
end
